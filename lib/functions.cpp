...
void initSSAA(int screenWidth, int screenHeight, GLuint ssaaFBO, GLuint ssaaColorBuffer, GLuint ssaaDepthBuffer) {
    int ssaaWidth = screenWidth * 2;  // Facteur de super-échantillonnage de 2x
    int ssaaHeight = screenHeight * 2;

    // Générer le framebuffer
    glGenFramebuffers(1, &ssaaFBO);
    glBindFramebuffer(GL_FRAMEBUFFER, ssaaFBO);

    // Créer la texture pour le color buffer
    glGenTextures(1, &ssaaColorBuffer);
    glBindTexture(GL_TEXTURE_2D, ssaaColorBuffer);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, ssaaWidth, ssaaHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, ssaaColorBuffer, 0);

    // Créer un renderbuffer pour le depth buffer
    glGenRenderbuffers(1, &ssaaDepthBuffer);
    glBindRenderbuffer(GL_RENDERBUFFER, ssaaDepthBuffer);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, ssaaWidth, ssaaHeight);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, ssaaDepthBuffer);

    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        //std::cerr << "SSAA Framebuffer not complete!" << std::endl;
    }
...
void renderSSAAResultToScreen(int screenWidth, int screenHeight, GLuint ssaaColorBuffer, Shader *ssaa, GLuint screenQuadVAO) {
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    glViewport(0, 0, screenWidth, screenHeight);  // Remettre à jour le viewport pour la résolution de l'écran
    ...
    glBindTexture(GL_TEXTURE_2D, ssaaColorBuffer);  // Texture rendue à haute résolution
